 ...
 */
SHMEMM0 /*static*/ struct ipc_queue queue_m0 = {0};	// ws x: no static for debugging
...
#ifdef USE_FROM_ISR		// ws
/* Macro that waits till TX/RX event to happen */
#define ipc_wait_event(evt, sem) while ((evt)) xSemaphoreTakeFromISR(sem, 0)
#define ipc_wait_event_tout(evt, tout, sem)	\
	do { \
		while ((evt)) {	\
			if ((evt) && xSemaphoreTakeFromISR(sem, 0) != pdTRUE) \
			{tout = 0; break; }} \
	} while (0)
#else
/* Macro that waits till TX/RX event to happen */
